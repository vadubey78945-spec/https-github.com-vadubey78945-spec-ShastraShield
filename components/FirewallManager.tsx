    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <div className="px-4 py-2 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-3">
            <i className="fas fa-microchip text-amber-500"></i>
            <div>
               <p className="text-[9px] text-slate-500 uppercase font-black">Segment Status</p>
               <p className="text-xs font-black text-amber-500 uppercase">Zero Trust Enforced</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <table className="w-full text-left border-separate border-spacing-y-3">
                  ...
                  <tbody>
                    {rules.filter(r => !isExpired(r)).map(rule => (
                      <tr key={rule.id} className="bg-slate-900/40 border border-white/5 group hover:border-cyan-500/20 transition-all">
                        <td className="px-4 py-4 rounded-l-2xl">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-xs text-slate-500 group-hover:text-cyan-400">
                        ...
                        <td className="px-4 py-4">
                           <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${
                             ...
                             rule.action === 'THROTTLE' ? 'bg-amber-500 text-white' : 'bg-emerald-500 text-white'
                           }`}>
                             {rule.action}
                           </span>
                        </td>
                        <td className="px-4 py-4">
                           <div className="flex flex-col">
                              <span className="text-[9px] font-mono text-cyan-500 uppercase">{rule.direction}</span>
        ...
        <div className="space-y-6">
           <div className="glass p-8 rounded-[2.5rem] border-cyan-500/20 bg-cyan-500/5 h-full">
              <h3 className="text-xs font-black text-cyan-500 uppercase tracking-[0.2em] mb-8">
                 Context IQ Engine
              </h3>
              
              <div className="space-y-6">
                 <div className="p-5 rounded-3xl bg-slate-950/40 border border-white/5">
                    ...
                    <div className="flex items-end justify-between">
                       <div className="flex flex-col">
                          ...
                          <span className="text-[9px] font-bold text-cyan-500 uppercase mt-1">AI-Authored Rules</span>
                       </div>
                       <i className="fas fa-robot text-cyan-500/30 text-4xl"></i>
                    </div>
                 </div>
